country_overall_percent_boxplot <- function(iiss_afghan){
  ...
  for(yr in years){
    ...
    for(state in countries){
      ...
      tot <- temp$troops_afghan_total/temp$troops_total
      percent <- tot*100
      if(length(percent) > 0){
        if(percent > 1.8){
  ...
  my_df <- my_df[which(my_df$percent > 0.00000000),]
  ...
  g <- ggplot(my_df, aes(x=my_df$year, y=my_df$percent,group=my_df$year)) +
    ...
    labs(title="Percent country contributions to ISAF by year",
         subtitle="2001-2005",
