...
##
model1 <- glm(contributor ~ sequiv + abs_ideal_diff + dem,
...
ggplot(isaf_contributors, aes(x = sequiv, y = troops_afghan_ratio)) +
...
model7 <- glm(log(troops_afghan_ratio) ~ sequiv + abs_ideal_diff +
              clustercoef + degree + dem, data = isaf_contributors)
broom::tidy(model7)

model8 <- plm::plm(log(troops_afghan_ratio) ~ sequiv + abs_ideal_diff +
                  clustercoef + degree + dem,
                  index = c("year", "country"), data = isaf_contributors)
summary(model8)
