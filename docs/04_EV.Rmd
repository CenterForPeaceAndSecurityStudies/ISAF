...
ideal <- dplyr::select(ideal, c(ccode, year, CountryName, Idealpoint, 
    PctAgreeUS))
...
isaf_ext$depth <- isaf_ext$depth + abs(summary(isaf_ext$depth)[1]) 
## Divide by max to make between 0 and 1
isaf_ext$depth <- isaf_ext$depth/summary(isaf_ext$depth)[6]
```

+ Create potential gain variable:

```{R}
isaf_ext$pot_gain <- isaf_ext$pctagreeus - isaf_ext$depth
```
...
+ Keeping this for later in case.

## Lebovic and Saunders

Second, let's add the the Lebovic and Saunders data on diplomatic visits. This only goes to 2012. **Update**: Let's hold off on using this data. 
...
dipl <- dplyr::select(diplomatic, c(cowid, year, bi_SOS, bi_PRE, bi_str_SOS,
    bi_str_PRE, USmilaid, allies, USdefense, UStrade, USalign, UNpart))

## colnames that make more sense
colnames(dipl) <- c("ccode", "year", "sec_st", "pres", "sec_only", "pres_only",
    "us_mil_aid", "us_ally", "us_def_pact", "us_trade", "us_un_vote_sim", 
    "un_particip")
...
+ **Update:** Same thing, let's hold off on using for now.
...
Let's also build the alliance network for each year so that we can calculate statistics for network position. Note, it is OK that this only includes states with an alliance. Any state not in the alliance network will be categorized as an NA for network statistics. **Update:** This is not necessary for the final models, but let's keep the code in case. 
...
year_states <- dplyr::select(year_states, c(stateabb, ccode, statenme, styear, 
    endyear))
...
We can now calculate statistics for a node's network position each year and input them into the isaf dataset. **Again, don't use for final project.**
