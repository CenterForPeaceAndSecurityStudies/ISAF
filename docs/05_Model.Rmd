...
+ Iceland -- drop for now until we figure out total available troops.
...
## Rescale to be between 0 and 1 via min/max
isaf$pot_gain_recode = (isaf$pot_gain_recode - 
  min(isaf$pot_gain_recode))/(
  max(isaf$pot_gain_recode)-min(isaf$pot_gain_recode))  
```
...
```{R}
troop_dist <- ggplot(isaf, aes(x = troops_afghan_ratio)) + 
...
```{R}
m1 <- glm(
  troops_afghan_ratio ~ pot_gain_recode, 
  data = isaf
  ) 
summary(m1)
...
m2 <- glm(
  troops_afghan_ratio ~ pot_gain_recode + idealpoint + dem + casualties + us_ally + cinc,
  data = isaf
  )
summary(m2)
...
m3 <- glm(
  troops_afghan_ratio ~ pot_gain_recode + idealpoint + dem + casualties + us_ally + cinc + log(gdp) + log(gdppc) + distance,
  data = isaf
  )
summary(m3)
...
m4 <- plm::plm(
  troops_afghan_ratio ~ pot_gain_recode + idealpoint + 
...
dw1 <- dwplot(
  list(m1, m2, m3, m4), dot_args = list(size = 2.5),
    ...
    relabel_predictors(c(
        pot_gain_recode = "Potential Gain With U.S.",
    ...
    theme_bw() +
    theme(text = element_text(size = 24, family = myfont1),
        axis.text.x = element_text(colour="black"), 
        axis.text.y = element_text(colour="black"),
