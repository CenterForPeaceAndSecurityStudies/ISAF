...
```{R}
distance <- rio::import('inst/extdata/distance.csv')

## Filter only by Afghanistan dyads
dist <- dplyr::filter(distance, year == 2001)
dist <- dplyr::filter(dist, ccode2 == 700 | ccode1 == 700)
## Strange setup...
distance <- dplyr::tibble(ccode = as.integer(),
                      distance = as.integer())
## just include all observations, in case there are some new observations in later years
for(i in 1:nrow(dist)){
    if(dist[i,1] == 700){
        distance[i,1] <- dist[i,2]
        distance[i,2] <- dist[i,4]
    }else{
        distance[i,1] <- dist[i,1]
        distance[i,2] <- dist[i,4]
    }
}

## Left join
isaf <- dplyr::left_join(isaf, distance)
```
...
Now we can merge everything together. Though first, have to merge country-years with two observations on troops. This happened because iiss lists some deployments separately.

```{R}
iiss_troops$ccode <- as.integer(iiss_troops$ccode)
iiss_troops$country <- as.character(iiss_troops$country)
troops <- dplyr::tibble(year = as.integer(),
                        country = as.character(),
                        ccode = as.integer(),
                        troops_total = as.integer(),
                        troops_afghan_total = as.integer(),
                        troops_afghan_oef = as.integer(),
                        troops_afghan_isaf = as.integer(),
                        troops_afghan_unama = as.integer(),
                        troops_afghan_ratio = as.numeric())

for(i in 1:nrow(iiss_troops)){
    ## If the same country year repeatedly
    if(iiss_troops[i,1] == iiss_troops[i+1,1] &
       iiss_troops[i,3] == iiss_troops[i+1,3]){
        ## Fill in each cell
        ## Text
        for(j in 1:3){
            troops[i,j] <- iiss_troops[i,j]
        }
        ## Numbers
        for(j in 4:8){
            troops[i,j] <- iiss_troops[i,j] + iiss_troops[i+1,j]
        }
       }else{
    ## Otherwise keep the same
    troops[i,] <- iiss_troops[i,]
       }
}
## Now let's add the last row
troops[2405,] <- iiss_troops[2405,]
## And then delete duplicates...
troops <- troops[!duplicated(troops[c("year", "ccode")]),]
##  Now let's recalculate ratios, since are NAs on merged obs.
for(i in 1:nrow(troops)){
    troops[i,9] <- troops[i,5]/troops[i,4]
}
## Rename
iiss_troops <- troops
final_data <- dplyr::left_join(iiss_troops, isaf)
...
final_data <- dplyr::select(final_data, c(year, country, stateabb, ccode,
        troops_total, troops_afghan_total, troops_afghan_oef, troops_afghan_isaf,
        troops_afghan_unama, troops_afghan_ratio, milex, milper, irst, pec, tpop,
        upop, cinc, distance, idealpoint, pctagreeus, us_un_vote_sim, un_particip, sec_st,
        pres, sec_only, pres_only, us_mil_aid, us_ally, us_def_pact,
        us_trade, dca, dca_ongoing, abs_ideal_diff, arms_imports, NATO,
        democ, autoc, polity2, dem, eigen, clustercoef, degree, sequiv))
```
...
>>>>>>> 5ee7cec2254634781d491b35f2d384eff5072203
