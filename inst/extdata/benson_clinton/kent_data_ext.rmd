...
+ Benson and Clinton drop obs that are only non-aggression pacts.
...
+ Ready data -- this is member-alliance observations.
    + **Problem: compellent, deterrence, and unconditional variables are all hand-coded by Benson in an earlier paper.** So I can't extend them with ATOP. I'll write him in the meantime, but am not hopeful.
...
+ Remove observations not in original paper:

```{R}
## test <- all_alliances[8:1766,]
```

+ Last, for merging purposes, let's drop NA observations.

```{R}
test <- tidyr::drop_na(all_alliances)
```
...
```{R}
original <- readr::read_csv("AllianceDataScore.csv")
```
...
+ Take average of scope and depth for each alliance with the US. Sum and standardize. How to best standardize.

+ Take dyadic data and select all relevant alliances, then sum and standardize.
