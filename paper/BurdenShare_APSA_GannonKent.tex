...
\section{Research Design}
	...
	\subsection{Model}
	 Our unit of analysis is the country-year, spanning from 2001-2005. Though we include measures of network statistics, because our outcome of interest is not tie-formation, but a node-level attribute, we employ a linear regression. For each dependent variable, we estimate a baseline `network only' model and then a second with controls for distance and democracy. The former captures the theoretical prediction of the balance of threats hypothesis. We include a control for democracy since some have argued that democracies are less reliable allies and more likely to pull out of coalition conflicts early \citep{massie_whydemocraticallies_2016, gartzke_whydemocraciesmay_2004}. While a slightly different context than our argument, the logical extension of those findings should predict that democracies are less likely to over-contribute. For each model we estimate with and without fixed effects, in order to control for unobserved time-invariate confounders.
